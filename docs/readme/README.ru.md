...
Для target bridge один раз загрузите расширение из `extensions/chrome`, затем выполните `/browser pair`. Chrome блокирует `chrome-extension://` URL, открываемые внешними программами (ERR_BLOCKED_BY_CLIENT — одинаково на macOS, Windows и Linux), поэтому попытка самого zode открыть страницу может не сработать — вместо этого расширение само открывает свою страницу pairing в течение ~30 секунд после `/browser pair`, с уже предзаполненным портом; введите там 6-значный pairing-код, показанный в чате. Запасной вариант: наберите URL `chrome-extension://…/popup.html?port=…` в адресной строке вручную (набранная вручную навигация исходит от браузера и не блокируется). **Pairing выполняется один раз**: расширение сохраняет долговременный token и переподключается автоматически — при старте браузера, при обновлениях расширения и с повтором примерно каждые 30 секунд, пока соединение потеряно, — так что перезапуск zode не требует нового pairing. Оно переподключается к запущенной CLI или автоматически стартует extension-only daemon Zode при необходимости. Вкладки, открытые zode, помещаются в Chrome tab group с именем `zode`.
...
Все наборы зелёные; сквозной самотест прогоняет реальный цикл эволюции —
приспособленность групп инструментов → сгенерированные JS-гены → отбор по ёмкости →
персистентность генома — и выводит `SELF-TEST PASSED`:

| Набор | Команда | Результат |
|---|---|---|
| Ядро harness, слой эволюции, процесс-плагины | `cargo test -p cordis-rs` | 50 passed |
| Интеграция эволюции (приспособленность групп, восстановление генома) | `cargo test -p zode-core --lib evolution::` | 5 passed |
| Слой генов QuickJS (замена кода, прерывание, лимит памяти) | `cargo test -p zode-core --test js_plugin_it` | 4 passed |
| Полный набор zode-core (включая подключение эволюции) | `cargo test -p zode-core --lib` | 983 passed |

```sh
cargo run -p zode-core --example evolution_self_test
```

- Конвейер хуков оценивает каждый результат инструмента по его группе
  (`uses − 10·failures − 100·panics − 5·restarts`); `unfit_groups()` перечисляет группы,
  которые стоит отключить.
- Пул генов имеет жёсткий лимит ёмкости: когда агент порождает новых кандидатов,
  слабейшие гены вытесняются (самотест вытесняет `git` → `todo` → `shell`); выживают
  наиболее приспособленные.
- Сгенерированные гены — это JavaScript (компилятор не нужен) с лимитом памяти и
  дедлайном прерывания на ген; вышедший из-под контроля ген изолируется, а не
  вредит zode.
- Геном сохраняется в `<config-dir>/evolution/genome.json` и восстанавливается вместе
  с приспособленностью после перезапуска; `dispose()` освобождает все файберы,
  слушатели и записи истории.

Полный отчёт с наблюдаемым выводом и исправленными регрессиями:
