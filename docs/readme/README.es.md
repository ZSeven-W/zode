...
Para el target bridge, carga la extensión una vez desde `extensions/chrome`, luego ejecuta `/browser pair`. Chrome bloquea las URLs `chrome-extension://` abiertas por programas externos (ERR_BLOCKED_BY_CLIENT, por igual en macOS, Windows y Linux), así que el intento del propio zode de abrir la página puede fallar — en su lugar, la propia extensión abre su página de emparejamiento en unos ~30 segundos tras `/browser pair`, con el puerto ya rellenado; introduce ahí el código de emparejamiento de 6 dígitos que se muestra en el chat. Como alternativa manual, escribe tú mismo la URL `chrome-extension://…/popup.html?port=…` en la barra de direcciones (la navegación tecleada a mano la inicia el navegador y está permitida). **El emparejamiento se hace una sola vez**: la extensión almacena un token de larga duración y se reconecta automáticamente — al arrancar el navegador, al actualizarse la extensión y con un reintento aproximadamente cada 30 segundos mientras esté desconectada — así que reiniciar zode nunca vuelve a pedirte emparejar. Se reconecta a una CLI en ejecución o auto-inicia un daemon zode exclusivo de la extensión cuando hace falta. Las pestañas que abre zode se colocan en un grupo de pestañas de Chrome llamado `zode`.
