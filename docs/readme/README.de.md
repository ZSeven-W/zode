...
Für das bridge-Ziel lädst du die Erweiterung einmal aus `extensions/chrome` und führst dann `/browser pair` aus. Chrome blockiert `chrome-extension://`-URLs, die von externen Programmen geöffnet werden (ERR_BLOCKED_BY_CLIENT – auf macOS, Windows und Linux gleichermaßen), zodes eigener Versuch, die Seite zu öffnen, kann also fehlschlagen – stattdessen öffnet die Erweiterung selbst innerhalb von ~30 Sekunden nach `/browser pair` ihre Pairing-Seite mit vorausgefülltem Port; gib dort den im Chat angezeigten 6-stelligen Pairing-Code ein. Als manueller Ausweg funktioniert es auch, die `chrome-extension://…/popup.html?port=…`-URL selbst in die Adressleiste einzutippen (manuell eingetippte Navigation gilt als vom Browser ausgelöst und ist erlaubt). **Das Pairing ist einmalig**: Die Erweiterung speichert ein langlebiges Token und verbindet sich automatisch neu – beim Browser-Start, bei Erweiterungs-Updates und in einem Retry-Rhythmus von etwa 30 Sekunden, solange die Verbindung getrennt ist – ein zode-Neustart verlangt also nie ein erneutes Pairing. Sie verbindet sich mit einer laufenden CLI wieder oder startet bei Bedarf automatisch einen erweiterungsseitigen zode-Daemon. Von zode geöffnete Tabs werden in einer Chrome-Tab-Gruppe namens `zode` platziert.
