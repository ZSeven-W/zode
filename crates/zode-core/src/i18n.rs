...
const EXTRA: &[[&str; 15]] = &[
    ...
    ["cache: {what} changed since the last turn — the prompt-cache prefix re-writes once", "缓存：{what} 自上一轮起已变化 — 提示词缓存前缀将重写一次", "快取：{what} 自上一輪起已變化 — 提示詞快取前綴將重寫一次", "キャッシュ: {what} が前のターンから変化 — プロンプトキャッシュのプレフィックスを一度再書き込み", "캐시: {what}이(가) 지난 턴 이후 변경됨 — 프롬프트 캐시 접두사가 한 번 다시 기록됨", "caché: {what} cambió desde el último turno — el prefijo de caché se reescribe una vez", "cache : {what} a changé depuis le dernier tour — le préfixe du cache est réécrit une fois", "Cache: {what} hat sich seit dem letzten Zug geändert — das Cache-Präfix wird einmal neu geschrieben", "cache: {what} mudou desde o último turno — o prefixo do cache é reescrito uma vez", "кэш: {what} изменился с прошлого хода — префикс кэша будет перезаписан один раз", "कैश: {what} पिछले टर्न से बदल गया — प्रॉम्प्ट-कैश प्रीफ़िक्स एक बार फिर लिखा जाएगा", "cache: {what} berubah sejak giliran terakhir — prefiks cache ditulis ulang sekali", "แคช: {what} เปลี่ยนไปตั้งแต่เทิร์นที่แล้ว — พรีฟิกซ์แคชจะถูกเขียนใหม่หนึ่งครั้ง", "önbellek: {what} son turdan beri değişti — önbellek öneki bir kez yeniden yazılır", "bộ nhớ đệm: {what} đã thay đổi từ lượt trước — tiền tố cache được ghi lại một lần"],
    ["Start fresh — clear the conversation context (alias of /clear)", "重新开始 — 清空会话上下文（/clear 的别名）", "重新開始 — 清空會話上下文（/clear 的別名）", "最初から — 会話コンテキストをクリア（/clear の別名）", "새로 시작 — 대화 컨텍스트 지우기 (/clear의 별칭)", "Empezar de cero — limpiar el contexto de la conversación (alias de /clear)", "Repartir de zéro — effacer le contexte de la conversation (alias de /clear)", "Neu anfangen — Gesprächskontext leeren (Alias von /clear)", "Começar do zero — limpar o contexto da conversa (alias de /clear)", "Начать заново — очистить контекст беседы (псевдоним /clear)", "नए सिरे से शुरू करें — वार्तालाप संदर्भ साफ़ करें (/clear का उपनाम)", "Mulai baru — bersihkan konteks percakapan (alias /clear)", "เริ่มใหม่ — ล้างบริบทการสนทนา (ชื่อแทนของ /clear)", "Sıfırdan başla — konuşma bağlamını temizle (/clear takma adı)", "Bắt đầu lại — xóa ngữ cảnh hội thoại (bí danh của /clear)"],
    ["weak-model behavior detected — lite accommodations will be enabled for this model (remembered for future sessions; set profile: \"standard\" in config to opt out)", "检测到弱模型行为 — 将为该模型启用 lite 适配（已记住，后续会话生效；在配置中设 profile: \"standard\" 可退出）", "偵測到弱模型行為 — 將為該模型啟用 lite 適配（已記住，後續會話生效；在設定中設 profile: \"standard\" 可退出）", "弱いモデルの挙動を検出 — このモデルに lite 対応を有効化（今後のセッションでも記憶；config の profile: \"standard\" で解除）", "약한 모델 동작 감지 — 이 모델에 lite 조정을 활성화 (향후 세션에도 기억; config에서 profile: \"standard\"로 해제)", "comportamiento de modelo débil detectado — se activarán las adaptaciones lite para este modelo (recordado para futuras sesiones; profile: \"standard\" en config para desactivar)", "comportement de modèle faible détecté — les adaptations lite seront activées pour ce modèle (mémorisé pour les sessions futures ; profile: \"standard\" dans la config pour refuser)", "Schwaches-Modell-Verhalten erkannt — Lite-Anpassungen werden für dieses Modell aktiviert (für künftige Sitzungen gemerkt; profile: \"standard\" in der Config zum Deaktivieren)", "comportamento de modelo fraco detectado — as adaptações lite serão ativadas para este modelo (lembrado para sessões futuras; profile: \"standard\" na config para desativar)", "обнаружено поведение слабой модели — для этой модели будут включены lite-адаптации (запомнено для будущих сессий; profile: \"standard\" в конфиге для отключения)", "कमज़ोर मॉडल व्यवहार का पता चला — इस मॉडल के लिए lite अनुकूलन सक्षम होंगे (भविष्य के सत्रों के लिए याद रखा गया; हटाने के लिए config में profile: \"standard\" सेट करें)", "perilaku model lemah terdeteksi — penyesuaian lite akan diaktifkan untuk model ini (diingat untuk sesi berikutnya; setel profile: \"standard\" di config untuk keluar)", "ตรวจพบพฤติกรรมโมเดลอ่อน — จะเปิดใช้การปรับ lite สำหรับโมเดลนี้ (จดจำไว้สำหรับเซสชันถัดไป; ตั้ง profile: \"standard\" ใน config เพื่อยกเลิก)", "zayıf model davranışı algılandı — bu model için lite uyarlamaları etkinleştirilecek (gelecek oturumlar için hatırlanır; devre dışı bırakmak için config'de profile: \"standard\")", "phát hiện hành vi mô hình yếu — sẽ bật các điều chỉnh lite cho mô hình này (được ghi nhớ cho các phiên sau; đặt profile: \"standard\" trong config để tắt)"],
    ["lite accommodations applied for this model", "已为该模型应用 lite 适配", "已為該模型套用 lite 適配", "このモデルに lite 対応を適用しました", "이 모델에 lite 조정을 적용했습니다", "adaptaciones lite aplicadas para este modelo", "adaptations lite appliquées pour ce modèle", "Lite-Anpassungen für dieses Modell angewendet", "adaptações lite aplicadas para este modelo", "lite-адаптации применены для этой модели", "इस मॉडल के लिए lite अनुकूलन लागू", "penyesuaian lite diterapkan untuk model ini", "ปรับ lite สำหรับโมเดลนี้แล้ว", "bu model için lite uyarlamaları uygulandı", "đã áp dụng điều chỉnh lite cho mô hình này"],
    ["auto-compact interrupted — paused for this session; run /compact to compact manually", "自动压缩已被打断 — 本会话内暂停；需要时用 /compact 手动压缩", "自動壓縮已被打斷 — 本會話內暫停；需要時用 /compact 手動壓縮", "自動圧縮が中断されました — このセッションでは一時停止；必要なら /compact で手動圧縮", "자동 압축이 중단됨 — 이 세션에서는 일시중지; 필요하면 /compact로 수동 압축", "compactación automática interrumpida — pausada en esta sesión; usa /compact para compactar manualmente", "compactage auto interrompu — en pause pour cette session ; utilisez /compact pour compacter manuellement", "Auto-Kompaktierung unterbrochen — für diese Sitzung pausiert; mit /compact manuell kompaktieren", "compactação automática interrompida — pausada nesta sessão; use /compact para compactar manualmente", "автосжатие прервано — приостановлено в этой сессии; используйте /compact для ручного сжатия", "स्वतः संक्षिप्तीकरण बाधित — इस सत्र में रोका गया; आवश्यकता होने पर /compact से मैन्युअल रूप से करें", "pemadatan otomatis terputus — dijeda untuk sesi ini; gunakan /compact untuk memadatkan manual", "การบีบอัดอัตโนมัติถูกขัดจังหวะ — หยุดชั่วคราวในเซสชันนี้; ใช้ /compact เพื่อบีบอัดเอง", "otomatik sıkıştırma kesildi — bu oturumda duraklatıldı; gerekirse /compact ile elle sıkıştırın", "nén tự động bị gián đoạn — tạm dừng trong phiên này; dùng /compact để nén thủ công"],
    ...
    ["context freed — resuming the interrupted task", "上下文已释放 — 恢复被打断的任务", "上下文已釋放 — 恢復被打斷的任務", "コンテキストを解放 — 中断されたタスクを再開します", "컨텍스트 확보 — 중단된 작업을 재개합니다", "contexto liberado — reanudando la tarea interrumpida", "contexte libéré — reprise de la tâche interrompue", "Kontext freigegeben — unterbrochene Aufgabe wird fortgesetzt", "contexto liberado — retomando a tarefa interrompida", "контекст освобождён — возобновление прерванной задачи", "संदर्भ खाली हुआ — बाधित कार्य फिर से शुरू हो रहा है", "konteks dibebaskan — melanjutkan tugas yang terputus", "เคลียร์บริบทแล้ว — กำลังทำงานที่ถูกขัดจังหวะต่อ", "bağlam boşaltıldı — kesilen görev sürdürülüyor", "đã giải phóng ngữ cảnh — tiếp tục tác vụ bị gián đoạn"],
    // ── Audit pass 2026-08: strings that shipped without table entries. ──
    ["browser config updated", "浏览器配置已更新", "瀏覽器設定已更新", "ブラウザ設定を更新しました", "브라우저 구성이 업데이트됨", "configuración del navegador actualizada", "configuration du navigateur mise à jour", "Browser-Konfiguration aktualisiert", "configuração do navegador atualizada", "конфигурация браузера обновлена", "ब्राउज़र कॉन्फ़िगरेशन अपडेट किया गया", "konfigurasi peramban diperbarui", "อัปเดตการตั้งค่าเบราว์เซอร์แล้ว", "tarayıcı yapılandırması güncellendi", "đã cập nhật cấu hình trình duyệt"],
    ["local operation id exhausted", "本地操作 ID 已耗尽", "本機操作 ID 已耗盡", "ローカル操作 ID を使い切りました", "로컬 작업 ID가 소진됨", "ID de operación local agotado", "ID d’opération locale épuisé", "Lokale Operations-ID erschöpft", "ID de operação local esgotado", "ID локальной операции исчерпан", "स्थानीय ऑपरेशन ID समाप्त", "ID operasi lokal habis", "ID การทำงานภายในหมดแล้ว", "yerel işlem kimliği tükendi", "đã hết ID thao tác cục bộ"],
    ["trust this session", "本会话内信任", "本會話內信任", "このセッションで信頼", "이 세션에서 신뢰", "confiar en esta sesión", "faire confiance pour cette session", "für diese Sitzung vertrauen", "confiar nesta sessão", "доверять в этой сессии", "इस सत्र में भरोसा करें", "percayai sesi ini", "เชื่อถือในเซสชันนี้", "bu oturumda güven", "tin cậy trong phiên này"],
