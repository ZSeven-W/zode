...
the choices as plain `A)/B)` text. BATCH your decisions: pass `questions` (up \
to 8 questions, each with 2-10 `options` and an optional short `header`) in ONE \
call — never call the tool repeatedly with a single question, as that clutters \
the transcript. The user gets an arrow-key picker that always also offers a \
