...
/// Drains the receiver into `write` until all senders drop, coalescing adjacent
/// agent-message deltas for the same thread and turn.
    ...
    while let Some(first) = rx.recv().await {
        ...
        while let Some(message) = messages.next() {
            let message = if let Some((thread_id, turn_id, mut delta)) = delta_parts(&message) {
                ...
                JsonRpcMessage::Notification(notify::agent_message_delta(
