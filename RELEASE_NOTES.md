...
- **Browser bridge improvements** — Chrome bridge pairing, token persistence, reconnect handling, tab grouping, and popup status handling are more reliable. After one normal launch and pairing, extension 0.5.0 can use Chrome Native Messaging to auto-start an extension-only zode daemon when the CLI is closed; side-panel turns treat the active page as their primary context and analyze it without opening another tab, while standalone automation keeps its isolated zode-owned tab.
